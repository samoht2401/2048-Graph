...
namespace Gui.Sprites
{
    public class Sprite
    {
        ...
        public void Draw(int x, int y, float scale = 1, float depth = 0, bool centered = false)
        {
            Draw(x, y, (int)(Width * scale), (int)(Height * scale), depth, centered);
        }
        public void Draw(int x, int y, int width, int height, float depth = 0, bool centered = false)
        {
            Texture.Bind();
            DrawHelper.Draw2DSprite(x, y, width, height, TexCoordRect, depth, centered);
        }
    }
}
